...
class InputConfiguration:
    def __init__(self):
        ...
        self.x0 = 0  # TODO: to fix x0 != 0
        self.p0 = 0  # TODO: to fix p0 != 0
        self.a = 1.0  # 1/a_0 -- for morse oscillator, a_0 -- for harmonic oscillator
        self.De = 20000.0  # 1/cm
        self.x0p = -0.17  # a_0
        self.E0 = 71.54  # 1/cm
        self.t0 = 300e-15  # s
        self.sigma = 50e-15  # s
        self.nu_L = 0.29297e15  # Hz
        ...
        self.delay = 600e-15  # s
        self.impulses_number = 1
        self.lmin = 0
        self.mod_stdout = 500
        self.mod_fileout = 100
        self.epsilon = 1e-15

        self.file_abs = "fort.21"
        self.file_real = "fort.22"
        self.file_mom = "fort.23"

        self.pot_type = PotentialType.MORSE
        self.wf_type = WaveFuncType.MORSE
        self.task = TaskType.TRANS_WO_CONTROL

    def load(self, data):
        # analyze provided input json file
        if "phys_syst_pars" in data:
            if "m" in data["phys_syst_pars"]: self.m = float(data["phys_syst_pars"]["m"])
        else:
        ...
        if "phys_calc_pars" in data:
            if "L" in data["phys_calc_pars"]: self.L = float(data["phys_calc_pars"]["L"])
            if "T" in data["phys_calc_pars"]: self.T = float(data["phys_calc_pars"]["T"])
            if "wf_type" in data["phys_calc_pars"]: self.wf_type = WaveFuncType[data["phys_calc_pars"]["wf_type"].upper()]
            if "task_type" in data["phys_calc_pars"]: self.task = TaskType[data["phys_calc_pars"]["task_type"].upper()]
        else:
        ...
        if "alg_calc_pars" in data:
            if "np" in data["alg_calc_pars"]: self.np = int(data["alg_calc_pars"]["np"])
            if "nch" in data["alg_calc_pars"]: self.nch = int(data["alg_calc_pars"]["nch"])
            if "nt" in data["alg_calc_pars"]: self.nt = int(data["alg_calc_pars"]["nt"])
            if "epsilon" in data["alg_calc_pars"]: self.epsilon = float(data["alg_calc_pars"]["epsilon"])
        else:
        ...
        if "init_conditions" in data:
            if "x0" in data["init_conditions"]: self.x0 = float(data["init_conditions"]["x0"])
            if "p0" in data["init_conditions"]: self.p0 = float(data["init_conditions"]["p0"])
        else:
        ...
        if "potential_pars" in data:
            if "a" in data["potential_pars"]: self.a = float(data["potential_pars"]["a"])
            if "De" in data["potential_pars"]: self.De = float(data["potential_pars"]["De"])
            if "x0p" in data["potential_pars"]: self.x0p = float(data["potential_pars"]["x0p"])
            if "pot_type" in data["potential_pars"]: self.pot_type = PotentialType[data["potential_pars"]["pot_type"].upper()]
        else:
        ...
        if "laser_field_pars" in data:
            if "E0" in data["laser_field_pars"]: self.E0 = float(data["laser_field_pars"]["E0"])
            if "t0" in data["laser_field_pars"]: self.t0 = float(data["laser_field_pars"]["t0"])
            if "sigma" in data["laser_field_pars"]: self.sigma = float(data["laser_field_pars"]["sigma"])
            if "nu_L" in data["laser_field_pars"]: self.nu_L = float(data["laser_field_pars"]["nu_L"])
            if "delay" in data["laser_field_pars"]: self.delay = float(data["laser_field_pars"]["delay"])
            if "impulses_number" in data["laser_field_pars"]: self.impulses_number = int(data["laser_field_pars"]["impulses_number"])
        else:
        ...
        if "print_pars" in data:
            if "lmin" in data["print_pars"]: self.lmin = int(data["print_pars"]["lmin"])
            if "mod_stdout" in data["print_pars"]: self.mod_stdout = int(data["print_pars"]["mod_stdout"])
            if "mod_fileout" in data["print_pars"]: self.mod_fileout = int(data["print_pars"]["mod_fileout"])
            if "file_abs" in data["print_pars"]: self.file_abs = data["print_pars"]["file_abs"]
            if "file_real" in data["print_pars"]: self.file_real = data["print_pars"]["file_real"]
            if "file_mom" in data["print_pars"]: self.file_mom = data["print_pars"]["file_mom"]
        else:
...
def main(argv):
    ...
    if not math.log2(conf.np).is_integer() or not math.log2(conf.nch).is_integer():
    ...
    if conf.lmin < 0 or conf.mod_fileout < 0 or conf.mod_stdout < 0 or conf.impulses_number < 0:
    ...
    if not conf.L > 0.0 or not conf.T > 0.0:
        raise ValueError("The value of spatial range 'L' and of time range 'T' of the problem"
                         "must be positive")

    if not conf.m > 0.0 or not conf.a > 0.0 or not conf.De > 0.0:
    ...
    if not conf.E0 >= 0.0 or not conf.sigma > 0.0 or not conf.nu_L >= 0.0:
    ...
    with open(os.path.join(OUT_PATH, conf.file_abs), 'w') as f_abs, \
         open(os.path.join(OUT_PATH, conf.file_real), 'w') as f_real, \
         open(os.path.join(OUT_PATH, conf.file_mom), 'w') as f_mom, \
         open(os.path.join(OUT_PATH, conf.file_abs) + "_exc", 'w') as f_abs_up, \
         open(os.path.join(OUT_PATH, conf.file_real + "_exc"), 'w') as f_real_up, \
         open(os.path.join(OUT_PATH, conf.file_mom + "_exc"), 'w') as f_mom_up, \
         open("test", 'w') as f:
        ...
        def report_static(stat: propagation.PropagationSolver.StaticState):
            ...
            emax0 = stat.v[0][1][0] + abs(stat.akx2[int(conf.np / 2 - 1)]) + 2.0
            emin0 = stat.v[0][0]
            ...
            np_min0 = int(
                math.ceil(conf.L *
                          math.sqrt(
                              2.0 * conf.m * (emax0 - emin0) * phys_base.dalt_to_au / phys_base.hart_to_cm) /
                          math.pi
                          )
            )
            ...
            nt_min0 = int(
                math.ceil((emax0 - emin0) * conf.T * phys_base.cm_to_erg / 2.0 / phys_base.Red_Planck_h
                          )
            )

            if conf.np < np_min0:
                _warning_collocation_points(conf.np, np_min0)
            if conf.nt < nt_min0:
                _warning_time_steps(conf.nt, nt_min0)

            cener0_tot = stat.cener0 + stat.cener0_u
            overlp0_abs = abs(stat.overlp00) + abs(stat.overlpf0)

            # plotting initial values
            plot(stat.psi0[0], 0.0, stat.x, conf.np)
            plot_up(stat.psi0[1], 0.0, stat.x, conf.np)
        ...
        def process_instrumentation(instr: propagation.PropagationSolver.InstrumentationOutputData):
            ...
            nt_min = int(math.ceil((instr.emax - instr.emin) * conf.T * phys_base.cm_to_erg / 2.0 / phys_base.Red_Planck_h))
            np_min = int(math.ceil(
                conf.L * math.sqrt(
                    2.0 * conf.m * (instr.emax - instr.emin) * phys_base.dalt_to_au / phys_base.hart_to_cm) / math.pi))
            ...
            if conf.task != TaskType.LOCAL_CONTROL:
                ...
                if dAdt >= 0.0:
                    ...
                    if abs(instr.psigc_psie.imag) > conf.epsilon:
                        ...
                        E_patched = dAdt_happy / (conf.epsilon * coef)
                    res = propagation.PropagationSolver.StepReaction.REPEAT

            # plotting the result
            if dyn_ref.l % conf.mod_fileout == 0 and res == propagation.PropagationSolver.StepReaction.OK:
                if dyn_ref.l >= conf.lmin:
                    plot(dyn_ref.psi[0], t, stat_saved.x, conf.np)
                    plot_up(dyn_ref.psi[1], t, stat_saved.x, conf.np)
            ...
            if dyn_ref.l % conf.mod_stdout == 0:
                if conf.np < np_min:
                    _warning_collocation_points(conf.np, np_min)
                if conf.nt < nt_min:
                    _warning_time_steps(conf.nt, nt_min)
        ...
        def LaserFieldEnvelope(stat: propagation.PropagationSolver.StaticState,
            ...
            if res_saved == propagation.PropagationSolver.StepReaction.OK:
                t = stat.dt * dyn.l
                E = phys_base.laser_field(conf.E0, t, conf.t0, conf.sigma)
                if conf.task == TaskType.INTUITIVE_CONTROL:
                    for npul in range(1, impulses_number):
                        E += phys_base.laser_field(conf.E0, t, conf.t0 + (npul * conf.delay), conf.sigma)
        ...
        solver = propagation.PropagationSolver(
            ...
            m=conf.m, L=conf.L, np=conf.np, nch=conf.nch, T=conf.T, nt=conf.nt,
            x0=conf.x0, p0=conf.p0, a=conf.a, De=conf.De, x0p=conf.x0p, E0=conf.E0,
            t0=conf.t0, sigma=conf.sigma, nu_L=conf.nu_L, delay=conf.delay)
